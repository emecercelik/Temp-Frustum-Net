...
CAR_OBJECT_CLASS = 0
PADDING_OBJECT_CLASS = 7

NUM_HEADING_BIN = 12
NUM_SIZE_CLUSTER = 8  # one cluster for each type
NUM_OBJECT_POINT = 512
g_type2class = {'Car': 0, 'Van': 1, 'Truck': 2, 'Pedestrian': 3,
                'Person_sitting': 4, 'Cyclist': 5, 'Tram': 6, 'Padding': 7}
g_class2type = {g_type2class[t]: t for t in g_type2class}
g_type2onehotclass = {'Car': 0, 'Pedestrian': 1, 'Cyclist': 2}

## These are KITTI data mean sizes of objects
g_type_mean_size = {'Car': np.array([3.85900386, 1.6279684, 1.49385419]),
                    ...
                    'Padding': np.array([0.00000001, 0.00000001, 0.00000001])}
...
def get_box3d_corners_helper(centers, headings, sizes, N=None):
...
def get_box3d_corners(center, heading_residuals, size_residuals, batch_size=None):
    """ TF layer.
    Inputs:
        center: (B,3)
...
def placeholder_inputs(batch_size, num_point, nr_channels, nr_classes=3):
    ...
    pointclouds_pl = tf.placeholder(tf.float32,
                                    shape=(batch_size, num_point, nr_channels))
    one_hot_vec_pl = tf.placeholder(tf.float32, shape=(batch_size, nr_classes))
...
def get_loss(mask_label, center_label, heading_class_label, heading_residual_label, size_class_label,
             size_residual_label, end_points, corner_loss_weight=10.0, box_loss_weight=1.0,cos_loss=False,
             cos_loss_weight=1.0):
    ...
    mask_loss = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(logits=end_points['mask_logits'], labels=mask_label))
    ...
    heading_residual_normalized_loss = huber_loss(
        tf.reduce_sum(end_points['heading_residuals_normalized'] * tf.to_float(hcls_onehot), axis=1) - \
    ...
    predicted_size_residual_normalized = tf.reduce_sum(end_points['size_residuals_normalized'] * scls_onehot_tiled,
                                                       axis=[1])  # Bx3
    ...
    if cos_loss:
        cosine_loss = cos_loss_weight * end_points['lstm_layer']['loss']
        tf.summary.scalar('cosine loss', cosine_loss)
    else:
        cosine_loss = 0.0
    
    # Weighted sum of all losses
    total_loss = mask_loss + box_loss_weight * (
                center_loss + heading_class_loss + size_class_loss + heading_residual_normalized_loss * 20 +
                size_residual_normalized_loss * 20 + stage1_center_loss + corner_loss_weight * corners_loss) + cosine_loss
