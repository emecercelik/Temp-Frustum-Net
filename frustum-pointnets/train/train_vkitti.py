...
import provider
from train_util import parse_batch

# from process_lstm_features import FEATURE_FILENAME
...
# from prepare_vkitti_data import NR_OBJECTS_PER_FRAME, NR_FRAMES
# BATCH_SIZE = NR_OBJECTS_PER_FRAME * NR_FRAMES
# BATCH_SIZE = 32

LOG_DIR = FLAGS.log_dir
if not os.path.exists(LOG_DIR): os.mkdir(LOG_DIR)
os.system('cp %s %s' % (MODEL_FILE, LOG_DIR))  # bkp of model def
os.system('cp %s %s' % (os.path.join(BASE_DIR, 'train_vkitti.py'), LOG_DIR))
os.system('cp %s %s' % (os.path.join(BASE_DIR, 'train_util.py'), LOG_DIR))
...
# Load Datasets
# overwritten_train_data_path = os.path.join(ROOT_DIR, 'kitti/rgb_vkitti_train.pickle')
# overwritten_val_data_path = os.path.join(ROOT_DIR, 'kitti/rgb_vkitti_validation.pickle')

TRAIN_DATASET = provider.VkittiFrustumDataset(npoints=NUM_POINT, split='train', rotate_to_center=True, random_flip=True,
                                              random_shift=True, one_hot=True)
TEST_DATASET = provider.VkittiFrustumDataset(npoints=NUM_POINT, split='validation', rotate_to_center=True, one_hot=True)
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            iou2ds, iou3ds = tf.py_func(provider.compute_box3d_iou, [end_points['center'], end_points['heading_scores'],
                                                                     ...
                                                                     centers_pl, heading_class_label_pl,
            ...
            correct = tf.equal(tf.argmax(end_points['mask_logits'], 2), tf.to_int64(labels_pl))
        ...
        ops = {'pointclouds_pl': pointclouds_pl,
               ...
               'global_features': end_points['global_features'],
               'bbox_features': end_points['bbox_features'],
...
def train_one_epoch(sess, ops, writer, nr_epoch):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_identifier, batch_frame_id, batch_object_id, \
        batch_data, batch_label, \
        batch_center, batch_hclass, batch_hres, \
        batch_sclass, batch_sres, \
        batch_rot_angle, batch_one_hot_vec = parse_batch(TRAIN_DATASET, train_idxs, start_idx, end_idx, NUM_POINT,
                                                         NUM_CHANNEL)
        ...
        summary, step, _, loss_val, logits_val, centers_pred_val, iou2ds, iou3ds, global_features, bbox_features, stage1_center = \
            sess.run([ops['merged'], ops['step'], ops['train_op'], ops['loss'], ops['logits'],
                      ops['centers_pred'], ops['end_points']['iou2ds'], ops['end_points']['iou3ds'],
                      ops['end_points']['global_features'], ops['end_points']['bbox_features'],
                      ops['end_points']['stage1_center']],
                     feed_dict=feed_dict)

        if (nr_epoch == MAX_EPOCH - 1):
            print(nr_epoch, ' = nr_epoch, dumping pickles')

            with open("global_features.pickle", 'ab') as fp:
                pickle.dump(batch_identifier, fp)
                pickle.dump(batch_frame_id, fp)
                pickle.dump(batch_object_id, fp)
                pickle.dump(batch_center, fp)
                pickle.dump(batch_hclass, fp)
                pickle.dump(batch_hres, fp)
                pickle.dump(batch_sclass, fp)
                pickle.dump(batch_sres, fp)
                pickle.dump(global_features, fp)

            with open("bbox_features.pickle", 'ab') as fp:
                pickle.dump(batch_identifier, fp)
                pickle.dump(batch_frame_id, fp)
                pickle.dump(batch_object_id, fp)
                pickle.dump(batch_center, fp)
                ...
                pickle.dump(batch_sclass, fp)
                pickle.dump(batch_sres, fp)
                pickle.dump(bbox_features, fp)

            with open("stage1_center.pickle", 'ab') as fp:
                pickle.dump(batch_identifier, fp)
                pickle.dump(batch_frame_id, fp)
                pickle.dump(batch_object_id, fp)
                pickle.dump(batch_center, fp)
    ...
    log_string('training segmentation avg class acc: %f' % (
...
def eval_one_epoch(sess, ops, writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_center, batch_hclass, batch_hres, \
        batch_sclass, batch_sres, \
        batch_rot_angle, batch_one_hot_vec = parse_batch(TEST_DATASET, test_idxs, start_idx, end_idx, NUM_POINT,
                                                         NUM_CHANNEL)
        ...
        summary, step, loss_val, logits_val, iou2ds, iou3ds = \
            sess.run([ops['merged'], ops['step'], ops['loss'], ops['logits'], ops['end_points']['iou2ds'],
    ...
    log_string('eval box IoU (ground/3D): %f / %f' % (
        iou2ds_sum / float(num_batches * BATCH_SIZE), iou3ds_sum / float(num_batches * BATCH_SIZE)))
    log_string(
        'eval box estimation accuracy (IoU=0.7): %f' % (float(iou3d_correct_cnt) / float(num_batches * BATCH_SIZE)))
