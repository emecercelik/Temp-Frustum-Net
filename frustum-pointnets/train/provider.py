...
from lstm_helper import gen_feature_dict, get_features, update_features
...
def rotate_pc_along_y(pc, rot_angle):
    ...
    Input:
        pc: numpy array (N,C), first 3 channels are XYZ
            z is facing forward, x is leftward, y is downward
...
class FrustumDataset(object):
    ...
    def __init__(self, npoints, split,
                 ...
                 overwritten_data_path=None, from_rgb_detection=False, one_hot=False, car_only=False,
                 tracks=False,tau=3,feat_len=515):
        '''
        Input:
            ...
            tracks: To get also track ids from the vkitti list
            tau: Number of frames to keep track of features
        '''
        ...
        if from_rgb_detection:
            with open(overwritten_data_path, 'rb') as fp:
                ...
                if tracks:
                    self.image_id_list = pickle.load(fp,encoding='latin1')
                    self.world_id_list = pickle.load(fp,encoding='latin1')
                    self.track_id_list = pickle.load(fp,encoding='latin1')
        else:
            with open(overwritten_data_path, 'rb') as fp:
                self.id_list = pickle.load(fp)
                self.box2d_list = pickle.load(fp,encoding='latin1')
                ...
                # frustum_angle is clockwise angle from positive x-axis
                self.frustum_angle_list = pickle.load(fp,encoding='latin1')
                # Emec added these four lines
                if tracks:
                    self.image_id_list = pickle.load(fp,encoding='latin1')
                    self.world_id_list = pickle.load(fp,encoding='latin1')
                    self.track_id_list = pickle.load(fp,encoding='latin1')
        # Generate feature dictionary using empty features, which will be filled in during training
        if tracks:
            self.w_l = np.array([int(i) for i in self.world_id_list])
            self.f_l = np.array([int(i) for i in self.image_id_list])
            self.t_l = np.array([int(i) for i in self.track_id_list])
            self.feature_dict = gen_feature_dict(self.w_l,self.f_l,self.t_l, features=None,tau=tau,feat_len=feat_len)
            drive_ids_list = list(set(self.w_l))
            self.drive_sizes = dict()
            for _w_id in drive_ids_list:
                _w_id_indices = np.where(self.w_l == _w_id)
                _f_list = self.f_l[_w_id_indices]
                self.drive_sizes[_w_id] = np.max(_f_list)
...
class VkittiFrustumDataset(object):
    ''' Dataset class for Frustum PointNets training/evaluation.
    Load prepared KITTI data from pickled files, return individual data element
    [optional] along with its annotations.
    '''

    def __init__(self, npoints, split, random_flip=False, random_shift=False, rotate_to_center=False,
        ...
        self.one_hot = one_hot
        if overwritten_data_path is None:
            overwritten_data_path = os.path.join(ROOT_DIR, 'kitti/rgb_vkitti_%s.pickle' % (split))
        ...
        if from_rgb_detection:
            with open(overwritten_data_path, 'rb') as fp:
                self.identifier_list = pickle.load(fp)
                self.frame_id_list = pickle.load(fp)
                self.object_id_list = pickle.load(fp)
    ...
    def __getitem__(self, index):
        ...
        identifier = self.identifier_list[index]
        ...
        if self.random_shift:
            dist = np.sqrt(np.sum(box3d_center[0] ** 2 + box3d_center[1] ** 2))
            shift = np.clip(np.random.randn() * dist * 0.05, dist * 0.9, dist * 1.1)
            point_set[:, 2] += shift
            box3d_center[2] += shift
        ...
        if self.one_hot:
            return identifier, frame_id, object_id, point_set, seg, box3d_center, angle_class, angle_residual, size_class, size_residual, rot_angle, one_hot_vec
        else:
            return identifier, frame_id, object_id, point_set, seg, box3d_center, angle_class, angle_residual, size_class, size_residual, rot_angle
...
def compute_box3d_iou(center_pred, heading_logits, heading_residuals, size_logits, size_residuals, center_label,
